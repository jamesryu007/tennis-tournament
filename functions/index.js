...
async function _doGenerateSpotlight(kst) {
  ...
  const dayGuide = [
    `올 시즌 구체적인 수치(승/패/승률)를 언급하되, 프로필의 개인 특성(성격·습관·에피소드)과 연결해 써줘. 숫자만 나열 금지.`,
    `실제 페어 전적에서 베스트 파트너를 구체적으로 언급하고, 프로필에서 그 파트너십이 잘 맞는 이유를 개인 특성과 연결해 써줘.`,
    `프로필에 나온 실제 에피소드·성격·생활 특성을 중심으로 써줘. 최근 경기와 자연스럽게 연결하면 더 좋아. 일반적인 칭찬 금지.`,
    `최근 경기 중 인상적인 장면을 상대팀 이름과 함께 구체적으로 묘사하고, 프로필의 개인 특성과 연결해 그 이유를 설명해줘.`,
    `프로필에서 이 멤버만의 독특한 특징이나 에피소드를 뽑고, 최근 경기나 시즌 성적과 연결해 독창적으로 써줘. 다른 멤버에게도 쓸 수 있는 표현 금지.`,
    `멤버 이름을 직접 불러 진심 어린 응원 메시지를 써줘. 프로필의 개인 특성을 언급해 이 멤버만을 위한 메시지임을 느끼게 해줘. 상투적 클로징 문구 금지.`,
    `오늘 코트에서 만난다는 설렘과 이 멤버의 프로필 특성(플레이 스타일·성격)을 엮어 써줘. 마지막 불릿에 "오늘 코트에서" 내용 필수.`,
  ][dow];
...
[출력 조건]
- 불릿 2~3개 (• 기호)
- 프로필(개인 특성·에피소드)을 반드시 1개 이상 불릿에 반영할 것
- 경기 기록이 없으면 프로필과 시즌 성적만으로 작성
