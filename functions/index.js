...
exports.fetchGolfPastWinner = onCall(
  ...
  async (req) => {
    ...
    try {
      ...
      console.error('fetchGolfPastWinner error:', e);
...
exports.fetchPlayerProfile = onCall({ region: 'asia-southeast1' }, async (req) => {
  ...
  // ── DB 캐시 확인 (90일 이내면 즉시 반환) ──────────────────────────────
  const safeKey = `${tour}_${name.replace(/[^a-zA-Z0-9]/g, '_')}`;
  const cacheRef = db.ref(`jmt/playerProfiles/${safeKey}`);
  try {
    const snap = await cacheRef.once('value');
    const dbCached = snap.val();
    if (dbCached && dbCached.fetchedAt && (Date.now() - dbCached.fetchedAt) < 90 * 24 * 60 * 60 * 1000) {
  ...
  // ── Wikipedia 서버사이드 fetch (사실 기반 컨텍스트 + 썸네일 사진) ────────
  let wikiContext = '';
  let thumbnailUrl = null;
  try {
    const wikiUrl = `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(name)}`;
    ...
    if (wikiRes.ok) {
      ...
      // Wikipedia 원본 썸네일 (800px 폭으로 업스케일)
      if (wikiData.originalimage?.source) {
        thumbnailUrl = wikiData.originalimage.source;
      } else if (wikiData.thumbnail?.source) {
        thumbnailUrl = wikiData.thumbnail.source.replace(/\/\d+px-/, '/400px-');
...
중요: 확실하지 않은 정보는 null. 국적·출생지·출생연도·프로데뷔연도는 최대한 채워줘. 코드블록 없이 JSON만 반환:
{"nationalityKo":"국적(한국어, 필수)","birthPlaceKo":"출생지 도시·국가 한국어 또는 null","birthYear":출생연도또는null,"turnedPro":프로데뷔연도또는null,${titlesField},"strengths":"기술적 강점 2~3가지 한국어 서술 또는 null","weaknesses":"약점·보완점 1~2가지 한국어 서술 또는 null","rivals":"주요 라이벌 선수명과 관계 한국어 서술 또는 null",${equipField},"careerHighlights":"주요 커리어 하이라이트 3~5개, 줄바꿈(\\n)으로 구분한 한국어(예:2019년 US오픈 첫 우승\\n2021년 세계랭킹 1위 달성) 또는 null","famousQuote":"선수 본인의 유명한 발언 한국어 번역 또는 null","triviaFacts":"팬들이 잘 모르는 흥미로운 사실 3~4가지 한국어(줄바꿈으로 구분, 사생활·취미·어린시절·독특한 기록 등 포함) 또는 null","playStyle":"플레이스타일 20자이내 한국어 또는 null","bio":"선수소개 2~3문장 한국어"}`;
  ...
  try {
    ...
    // DB에 저장 (90일 캐시) — viewers 서브노드 보존을 위해 update 사용
    cacheRef.update({ profile, thumbnailUrl, fetchedAt: Date.now() }).catch(e => console.error('playerProfile cache save error:', e));
...
// ── 스포트라이트 텍스트 생성 공통 로직 ─────────────────────────────
async function _doGenerateSpotlight(kst) {
  ...
  const dayGuide = [
    `올 시즌 성적(승/패/승률 수치)을 중심으로, 코트 밖 생활(프로필)과 자연스럽게 연결해 써줘. 페어·성격 언급 금지. 숫자만 나열 금지.`,
    `페어 전적 데이터에서 베스트 파트너 1명을 구체적(N승N패)으로 언급해줘. 성적 수치나 개인 직업 언급 금지.`,
    `프로필의 코트 밖 생활(직업·일상)을 중심으로 테니스와 연결해 써줘. 성적 수치·페어 전적 언급 금지. 일반적 칭찬 금지.`,
    `[최근 경기] 데이터가 있으면 반드시 1개 이상 사용해줘 — 날짜·파트너·상대·결과를 사실 그대로 언급. 나머지 불릿은 시즌 성적이나 프로필로. 기록에 없는 팀명·플레이 창작 절대 금지.`,
    `프로필에서 이 멤버만의 독특한 에피소드(연애·개인 특성 등)를 중심으로 써줘. 성적·페어·직업 언급 금지. 다른 멤버에게도 쓸 수 있는 표현 금지.`,
    `멤버 이름을 직접 불러 진심 어린 응원 메시지를 써줘. 프로필 요소 중 오늘까지 쓰지 않은 것을 하나 언급해줘. 상투적 클로징 금지.`,
    `오늘 모임 설렘을 담되, 프로필·시즌 성적·페어 중 딱 하나를 골라 연결해줘. 마지막 불릿에 "오늘 코트에서" 내용 필수.`,
  ][dow];
...
[출력 조건]
- 불릿 2개 기본, 내용이 풍부할 때만 3개
- 불릿마다 서로 다른 소재 사용 (같은 키워드 반복 금지)
- [멤버 정보]에 없는 내용 창작 금지 — 위반 시 심각한 오류
...
// 매일 자정 00:05 KST — 오늘의 스포트라이트 텍스트 미리 생성 (DB 캐시)
// 일요일: rotateSpotlightMember(00:00) 완료 후 새 멤버 기준으로 생성
exports.generateSpotlightDaily = onSchedule(
  { schedule: '5 0 * * *', timeZone: 'Asia/Seoul', region: 'asia-southeast1' },
  async () => {
    try {
      const kst = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Seoul' }));
      const result = await _doGenerateSpotlight(kst);
      if (result.error) {
        console.error('generateSpotlightDaily error:', result.error);
      } else {
        console.log(`spotlight daily generated: ${result.memberName} (${result.theme})`);
      }
    } catch (e) {
      console.error('generateSpotlightDaily error:', e);
    }
  }
);
